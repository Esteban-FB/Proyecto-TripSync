...
const LocalList = ({navigation}) => {
  ...
  // }, [user]);
      useFocusEffect(
        React.useCallback(() => {
          const fetchLocalesNoCoinciden = async () => {
            if (user) {
              try {
                const response = await axios.get(`http://10.0.2.2:5000/api/locales/getLocalesNoCoinciden/${user.user}`);
                console.log("Entra");
                setLocales(response.data);
              } catch (error) {
                console.error('Error al obtener la lista de locales', error);
...
<View style={styles.centeredView}>
    ...
    <View style={styles.localContainer}>
      <Swiper style={styles.wrapper} showsButtons={true}>
        <View style={styles.slide}>
          <Image source={require('../../assets/montana.jpg')} style={styles.image} />
        </View>
        <View style={styles.slide}>
        <Image
          style={styles.image}
          source={ require('../../assets/montana2.jpg' )}
          resizeMode="cover"
        />
      </View>
      <View style={styles.slide}>
        <Image
          style={styles.image}
          source={ require('../../assets/montana3.jpg' )}
          resizeMode="cover"
        />
      </View>
      </Swiper>
  ...
  // const toggleMostrarRecomendados = () => {
  //   setMostrarRecomendados(!mostrarRecomendados);
  // };
